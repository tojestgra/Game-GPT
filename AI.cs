        ...
        public static async Task<JArray> GetResponseBOI(string[] prompt, string[] system, Menu menu, int tokens, string GPT, int[] level)
        {
            ...
            else if (level[0] == 1)
            {
                ...
                else if (level[1] == 1)
                {
                    ...
                    string cleanresponse = await AIHandler.Get_response("Your goal is to add any important information from <|prompt|> that might or might not be in <|response|>. What constitutes important information ? Guidelines and strict formats for information should always be included. For example if the original prompt says that a certain string has to be formatted in a specific way, but the response omits that information, you have to put it back into the response\n\n\n"+prompt2 + "<|response|>" + response + "<|end|>", "Your goal is to add any important information from <|prompt|> that might or might not be in <|response|>. What constitutes important information ? Guidelines and strict formats for information should always be included", menu, tokens, GPT, false);
                    string response2 = await AIHandler.Get_response(cleanresponse,"",menu,tokens, GPT, false);
